...
class Pagamento:
    ...
    def set(self, importo):
        self.__import = importo

    def dettagliPagamento(self):
        return f"Importo del pagamento:{self.get():.2f}"
...
class PagamentoContanti(Pagamento):
    def __init__(self, importo: float):
        ...
        self.set(importo)
    def dettagliPagamento(self):
        return f"Pagamento in contanti è di:{self.get()}"
    
    def inPezzida(self):
        euro: dict = {500: 0, 200: 0, 100: 0, 50: 0, 20:0, 10:0, 5: 0, 2:0, 1:0, 0.5:0, 0.2:0, 0.1:0, 0.05: 0, 0.02:0, 0.01: 0}
        contanti = self.get()
        while contanti >= 0:
            for x in euro.keys():
                if contanti - x >= 0:
        ...
        for k,v in euro.items():
            if v> 0:
                print(f"{v} bancote da {k} euro")

pagamento1 = PagamentoContanti(1500)
...
class PagamentoCartadiCredito(Pagamento):
    def __init__(self, importo, nome_titolare, data_di_scadenza, numero_cc):
        super().__init__()
        self.set(importo)
    ...
    def dettagliPagamento(self):
        return f"Pagamento in contanti è di:{self.get():.2f}, \n Nome sulla carta: {self.nome_titolare}, \n Data di scadenza: {self.data_di_scadenza} \n Numero della carta: {self.numero_cc}"
